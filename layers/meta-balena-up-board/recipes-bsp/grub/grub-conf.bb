...
do_compile() {
    sed -e 's/@@TIMEOUT@@/3/' \
        -e 's/@@KERNEL_CMDLINE@@/rootwait processor.max_cstate=1 i915.enable_rc6=0/' \
        "${WORKDIR}/grub.cfg_internal_template" > "${B}/grub.cfg_internal-dev"

    sed -e 's/@@TIMEOUT@@/3/' \
        -e 's/@@KERNEL_CMDLINE@@/rootwait quiet loglevel=0 splash udev.log-priority=3 vt.global_cursor_default=0 processor.max_cstate=1 i915.enable_rc6=0/' \
